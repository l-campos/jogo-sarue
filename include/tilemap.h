...
private:
    std::vector<int> groundLayer; // "Camada de Blocos 1" (duas juntas)
    std::vector<int> climbLayer;  // "Elementos de escalar"
    std::vector<int> waterLayer;  // "agua"
    ...
    // NOVO: offset em pixels lido do .tmj (campo "offsetx"/"offsety" das
    // camadas). No mapa da fase 2 todas as camadas tem o mesmo offset
    // (192.667, 23.333), entao guardamos um unico par aqui.
    // Esse valor e adicionado as coordenadas de desenho E subtraido antes
    // de qualquer conversao pixel->tile, para que render e colisao batam.
    float mapOffsetX;
    float mapOffsetY;
