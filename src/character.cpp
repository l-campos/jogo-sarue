...
void Character::Update(float dt) {
    ...
    // ------------------------------------------------------------------
    // ÁGUA: se o centro do saruê tocar um tile da camada "água", ele morre
    // na hora. Reaproveita a mesma sequência de morte de baixo (queda pra
    // fora da tela) — não precisa de nada especial além de zerar o hp.
    // ------------------------------------------------------------------
    if (hp > 0 && TileMap::instance != nullptr) {
        Vec2 center = associated.box.Center();
        if (TileMap::instance->IsWaterAtPixel(center.x, center.y)) {
            hp = 0;
        }
    }
